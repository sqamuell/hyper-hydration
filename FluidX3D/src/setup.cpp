...
/*void main_setup() { // hydraulic jump
	// ######################################################### define simulation box size, viscosity and volume force ############################################################################
	LBM lbm(96u, 352u, 96u, 1u, 2u, 1u, 0.007f, 0.0f, 0.0f, -0.0005f);
	// #############################################################################################################################################################################################
	const ulong N=lbm.get_N(); const uint Nx=lbm.get_Nx(), Ny=lbm.get_Ny(), Nz=lbm.get_Nz(); for(ulong n=0ull; n<N; n++) { uint x=0u, y=0u, z=0u; lbm.coordinates(n, x, y, z);
		// ########################################################################### define geometry #############################################################################################
...
/*void main_setup() { // raindrop setup (#define D3Q19, SRT, VOLUME_FORCE, EQUILIBRIUM_BOUNDARIES, SURFACE) this is shit
...
/*void main_setup() { // bursting bubble setup
	...
	for(ulong n=0ull; n<N; n++) { uint x=0u, y=0u, z=0u; lbm.coordinates(n, x, y, z);
		...
		//if(cylinder(x,y,z, float3(lbm.center().x, lbm.center().y, (float)H - 0.5f * d), float3(lbm.center().x, lbm.center().y, (float)H - 0.5f * d), r + 1.0f)){
		if(sphere(x, y, z, float3(lbm.center().x, lbm.center().y, (float)H-0.5f*d), r+1.0f)) { // bubble
			//const float b = clamp(cylinder_x_plic(x, y, z, float3(lbm.center().x, lbm.center().y, (float)H - 0.5f * d), r, 0.5f * d), 0.0f, 1.0f);
