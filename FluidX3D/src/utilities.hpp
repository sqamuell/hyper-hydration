...
inline float half_to_float_custom(const ushort x)
...
inline ushort float_to_half_custom(const float x)
{																																																																																											// custom 16-bit floating-point format, 1-4-11, exp-15, +-1.99951168, +-6.10351562E-5, +-2.98023224E-8, 3.612 digits
...
inline float find_zero(float f(float), float min = -1.0f, float max = 1.0f, float offset = 0.0f)
{																												 // find zero of function f(x) in [min, max] by nested intervals
...
inline float integrate(float f(float), const float min, const float max, const uint N = 1000000u)
{ // do numeric integration of function f(x)dx in [min, max] by trapezoidal rule
...
inline float derivative(float f(float), const float x)
{ // compute central first derivative of f(x) at x
	const float dx = fmax(fabs(x), 1.0f) * 2.0f * epsilon_float;
	return (float)((double)f(x + dx) - (double)f(x - dx)) * 0.5f / dx;
}
inline float second_derivative(float f(float), const float x)
{ // compute central second derivative of f(x) at x
	const float dx = fmax(fabs(x), 1.0f) * 2.0f * epsilon_float;
...
inline void lu_solve(float *M, float *x, float *b, const int N)
{ // solves system of N linear equations M*x=b
	for (int i = 0; i < N; i++)
	{ // decompose M in M=L*U
		for (int j = i + 1; j < N; j++)
		{
			M[N * j + i] /= M[N * i + i];
			for (int k = i + 1; k < N; k++)
...
inline double find_zero(double f(double), double min = -1.0, double max = 1.0, double offset = 0.0)
{																																// find zero of function f(x) in [min, max] by nested intervals
...
inline double integrate(double f(double), const double min, const double max, const uint N = 1000000u)
{ // do numeric integration of function f(x)dx in [min, max] by trapezoidal rule
...
inline double derivative(double f(double), const double x)
{ // compute central first derivative of f(x) at x
	const double dx = fmax(fabs(x), 1.0) * 2.0 * epsilon_double;
	return (f(x + dx) - f(x - dx)) * 0.5 / dx;
}
inline double second_derivative(double f(double), const double x)
{ // compute central second derivative of f(x) at x
	const double dx = fmax(fabs(x), 1.0) * 2.0 * epsilon_double;
...
inline uint lcm(const uint x, const uint y)
...
inline ulong gcd(ulong x, ulong y)
{ // greatest common divisor
	if (x * y == 0ull)
		return 0ull;
	ulong t;
...
inline ulong lcm(const ulong x, const ulong y)
{ // least common multiple
...
inline float reverse_bytes(const float x)
...
inline double reverse_bytes(const double x)
{ // reverse the order of bytes
	double r;
	char *cx = (char *)&x;
	char *cr = (char *)&r;
...
inline char reverse_bytes(const char x)
{ // reverse the order of bytes
	return x;
}
inline uchar reverse_bytes(const uchar x)
{ // reverse the order of bytes
	return x;
}
inline short reverse_bytes(const short x)
{ // reverse the order of bytes
	short r;
	char *cx = (char *)&x;
	char *cr = (char *)&r;
...
inline ushort reverse_bytes(const ushort x)
{ // reverse the order of bytes
	ushort r;
	char *cx = (char *)&x;
	char *cr = (char *)&r;
...
inline int reverse_bytes(const int x)
{ // reverse the order of bytes
	int r;
	char *cx = (char *)&x;
	char *cr = (char *)&r;
...
inline uint reverse_bytes(const uint x)
{ // reverse the order of bytes
	uint r;
	char *cx = (char *)&x;
	char *cr = (char *)&r;
...
inline slong reverse_bytes(const slong x)
{ // reverse the order of bytes
	slong r;
	char *cx = (char *)&x;
	char *cr = (char *)&r;
...
inline ulong reverse_bytes(const ulong x)
{ // reverse the order of bytes
	ulong r;
	char *cx = (char *)&x;
	char *cr = (char *)&r;
...
{ // 3D vector type
	...
	inline int3 &operator+=(const int x)
	{ // elementwise addition
		this->x += x;
		this->y += x;
		this->z += x;
	...
	inline int operator*(const int3 &v) const
	{ // dot product
...
inline int3 operator+(const int3 &v, const int x)
{ // elementwise addition
	return int3(v.x + x, v.y + x, v.z + x);
}
inline int3 operator+(const int x, const int3 &v)
{ // elementwise addition
	return v + x;
}
inline int3 operator-(const int3 &v, const int x)
...
{ // 3D vector type
	...
	inline uint3 &operator+=(const uint x)
	{ // elementwise addition
		this->x += x;
		this->y += x;
		this->z += x;
	...
	inline uint operator*(const uint3 &v) const
	{ // dot product
		return this->x * v.x + this->y * v.y + this->z * v.z;
	}
...
inline uint3 operator+(const uint3 &v, const uint x)
{ // elementwise addition
	return uint3(v.x + x, v.y + x, v.z + x);
}
inline uint3 operator+(const uint x, const uint3 &v)
{ // elementwise addition
	return v + x;
}
inline uint3 operator-(const uint3 &v, const uint x)
...
{ // 3D vector type
	...
	inline float3 &operator+=(const float x)
	{ // elementwise addition
		this->x += x;
		this->y += x;
		this->z += x;
	...
	inline float operator*(const float3 &v) const
	{ // dot product
		return this->x * v.x + this->y * v.y + this->z * v.z;
	}
...
inline float3 operator+(const float3 &v, const float x)
{ // elementwise addition
	return float3(v.x + x, v.y + x, v.z + x);
}
inline float3 operator+(const float x, const float3 &v)
{ // elementwise addition
	return v + x;
}
inline float3 operator-(const float3 &v, const float x)
...
inline float3 xyz(const float3 &rtp)
{ // spherical coordinates (r, theta, phi) -> cartesian coordiantes (x, y, z)
	const float rsint = rtp.x * sin(rtp.y);
	return float3(rsint * cos(rtp.z), rsint * sin(rtp.z), rtp.x * cos(rtp.y));
}
inline float3 rtp(const float3 &xyz)
{ // cartesian coordiantes (x, y, z) -> spherical coordinates (r, theta, phi)
	const float r = length(xyz);
	return float3(r, (float)acos(xyz.z / r), (float)atan2(xyz.y, xyz.x));
}
inline float plane_distance(const float3 &p, const float3 &plane_point, const float3 &plane_normal)
...
{ // 3D matrix type
	...
	float3x3(const float xx, const float yy, const float zz)
	{ // create diagonal matrix
		this->xx = xx;
		this->yy = yy;
		this->zz = zz;
	...
	float3x3(const float xx, const float xy, const float xz, const float yx, const float yy, const float yz, const float zx, const float zy, const float zz)
	{ // create matrix
		this->xx = xx;
		this->xy = xy;
		this->xz = xz;
	...
	float3x3(const float3 &v1, const float3 &v2)
	{ // create matrix from dyadic product of two vectors
		this->xx = v1.x * v2.x;
		this->xy = v1.x * v2.y;
		this->xz = v1.x * v2.z;
		this->yx = v1.y * v2.x;
	...
	float3x3(const float3 &v1, const float3 &v2, const float3 &v3)
	...
	float3x3(const float3 &v, const float r)
	{ // create rotation matrix for a rotation around the (normalized) axis v with angle r in radians
		const float sr = sin(r), cr = cos(r);
		this->xx = sq(v.x) + (1.0f - sq(v.x)) * cr;
		this->xy = v.x * v.y * (1.0f - cr) - v.z * sr;
		this->xz = v.x * v.z * (1.0f - cr) + v.y * sr;
		this->yx = v.x * v.y * (1.0f - cr) + v.z * sr;
		this->yy = sq(v.y) + (1.0f - sq(v.y)) * cr;
		this->yz = v.y * v.z * (1.0f - cr) - v.x * sr;
		this->zx = v.x * v.z * (1.0f - cr) - v.y * sr;
		this->zy = v.y * v.z * (1.0f - cr) + v.x * sr;
		this->zz = sq(v.z) + (1.0f - sq(v.z)) * cr;
	}
	float3x3() = default;
	...
	inline float3x3 &operator+=(const float x)
	{ // elementwise addition
		this->xx += x;
		this->xy += x;
		this->xz += x;
	...
	inline float3x3 operator^(const uint n) const
	{																			 // matrix power
...
inline float3x3 operator+(const float3x3 &m, const float x)
{ // elementwise addition
	return float3x3(
...
inline float3x3 operator+(const float x, const float3x3 &m)
{ // elementwise addition
	return m + x;
}
inline float3x3 operator-(const float3x3 &m, const float x)
...
inline float3 operator*(const float3 &v, const float3x3 &m)
...
inline float3::float3(const float3x3 &m)
{ // extract diagonal of matrix
	this->x = m.xx;
	this->y = m.yy;
	this->z = m.zz;
}
inline float3 &float3::operator=(const float3x3 &m)
{ // extract diagonal of matrix
	this->x = m.xx;
	this->y = m.yy;
	this->z = m.zz;
...
struct floatN
{
	...
	float *V; // vector data
	floatN(const uint N, const float x = 0.0f)
	...
	inline floatN &operator+=(const float x)
	{ // elementwise addition
		for (uint i = 0u; i < N; i++)
			this->V[i] += x;
		return *this;
	...
	inline floatN operator+(const floatN &v) const
	{ // elementwise addition
		floatN r = floatN(N);
		for (uint i = 0u; i < N; i++)
	...
	inline float operator*(const floatN &v) const
	{ // dot product
		double r = 0.0;
		for (uint i = 0u; i < N; i++)
			r += (double)this->V[i] * (double)v.V[i];
		return (float)r;
	}
	inline string stringify() const
	{ // converts vector into string without spaces or newlines
...
inline floatN operator+(const floatN &v, const float x)
{ // elementwise addition
	floatN r = floatN(v.N);
	for (uint i = 0u; i < v.N; i++)
		r.V[i] = v.V[i] + x;
...
inline floatN operator+(const float x, const floatN &v)
{ // elementwise addition
	return v + x;
}
inline floatN operator-(const floatN &v, const float x)
...
struct floatNxN
{
	...
	float *M; // matrix data
	floatNxN(const uint N, const float x = 0.0f)
	...
	floatNxN(const floatN &v)
	{ // create diagonal matrix from vector
		this->N = v.N;
		this->M = new float[v.N * v.N];
		for (uint i = 0u; i < v.N * v.N; i++)
			this->M[i] = 0.0f;
		for (uint i = 0u; i < v.N; i++)
			this->M[v.N * i + i] = v.V[i];
	}
	floatNxN() = default;
	~floatNxN()
	{
	...
	inline floatNxN &operator=(const floatNxN &m)
	{
		...
		for (uint i = 0u; i < m.N * m.N; i++)
			this->M[i] = m.M[i];
		return *this;
	}
	inline floatNxN &operator=(const floatN &v)
	{ // create diagonal matrix from vector
		delete[] this->M;
		this->N = v.N;
		this->M = new float[v.N * v.N];
	...
	{ // matrix multiplication
		...
		for (uint i = 0u; i < N; i++)
		{
			for (uint j = 0u; j < N; j++)
			{
				double t = 0.0;
				for (uint k = 0u; k < N; k++)
					t += (double)B.M[N * i + k] * (double)m.M[N * k + j];
				this->M[N * i + j] = (float)t;
			}
		}
		return *this;
	}
	inline floatNxN &operator+=(const float x)
	{ // elementwise addition
		for (uint i = 0u; i < N * N; i++)
			this->M[i] += x;
		return *this;
	...
	inline floatNxN operator-() const
	{ // elementwise negation
	...
	inline floatNxN operator+(const floatNxN &m) const
	{ // elementwise addition
		floatNxN r = floatNxN(N);
		for (uint i = 0u; i < N * N; i++)
	...
	{ // matrix multiplication
		...
		for (uint i = 0u; i < N; i++)
		{
			for (uint j = 0u; j < N; j++)
			{
				double t = 0.0;
				for (uint k = 0u; k < N; k++)
					t += (double)this->M[N * i + k] * (double)m.M[N * k + j];
				r.M[N * i + j] = (float)t;
			}
		}
		return r;
	}
	inline floatNxN operator^(const uint n) const
	{																								// matrix power
	...
	{ // returns inverse matrix
		...
		{
			for (uint j = 0u; j < N; j++)
				A[2u * N * i + j] = (double)M[N * i + j];
			for (uint j = N; j < 2u * N; j++)
				A[2u * N * i + j] = (double)(i + N == j);
		}
		for (uint k = 0u; k < N - 1u; k++)
		{
			...
			for (uint i = k + 1u; i < N; i++)
			{
				...
				for (uint j = k; j < 2u * N; j++)
					A[2u * N * i + j] -= A[2u * N * k + j] * t;
			}
		}
		double det = 1.0;
		for (uint k = 0u; k < N; k++)
		...
		for (uint i = 0u; i < N; i++)
		{
			...
			for (uint j = 0u; j < N; j++)
				r.M[N * i + j] = (float)(A[2u * N * i + N + j] / t);
		}
		delete[] A;
		return r;
	}
	inline floatNxN transpose() const
	...
	inline string stringify() const
	{ // converts matrix into string without spaces or newlines
		string s = "{" + to_string(M[0]);
		for (uint i = 1u; i < N * N; i++)
			s += "," + to_string(M[i]);
		return s + "}";
	}
};
inline floatNxN operator+(const floatNxN &m, const float x)
{ // elementwise addition
...
inline floatNxN operator+(const float x, const floatNxN &m)
{ // elementwise addition
	return m + x;
}
inline floatNxN operator-(const floatNxN &m, const float x)
...
inline floatN operator*(const floatN &v, const floatNxN &m)
{ // multiply matrix with vector
	floatN r = floatN(v.N);
	for (uint i = 0u; i < v.N; i++)
	{
		double t = 0.0;
		for (uint j = 0u; j < v.N; j++)
			t += (double)m.M[m.N * i + j] * (double)v.V[j];
		r.V[i] = (float)t;
	}
	return r;
}
inline floatN operator*(const floatNxN &m, const floatN &v)
{ // multiply vector with matrix
	floatN r = floatN(m.N);
	for (uint i = 0u; i < m.N; i++)
	{
		double t = 0.0;
		for (uint j = 0u; j < m.N; j++)
			t += (double)v.V[j] * (double)m.M[m.N * j + i];
		r.V[i] = (float)t;
	}
	return r;
}
inline floatN::floatN(const uint N, const floatNxN &m)
{ // extract diagonal of matrix
	this->N = m.N;
	this->V = new float[m.N];
	for (uint i = 0u; i < m.N; i++)
		this->V[i] = m.M[m.N * i + i];
}
inline floatN &floatN::operator=(const floatNxN &m)
{ // extract diagonal of matrix
	delete[] this->V;
	this->N = m.N;
	this->V = new float[m.N];
...
{ // 3D vector type
	...
	inline double3 &operator+=(const double x)
	{ // elementwise addition
		this->x += x;
		this->y += x;
		this->z += x;
	...
	inline double operator*(const double3 &v) const
	{ // dot product
		return this->x * v.x + this->y * v.y + this->z * v.z;
	}
...
inline double3 operator+(const double3 &v, const double x)
{ // elementwise addition
	return double3(v.x + x, v.y + x, v.z + x);
}
inline double3 operator+(const double x, const double3 &v)
{ // elementwise addition
	return v + x;
}
inline double3 operator-(const double3 &v, const double x)
...
inline double3 xyz(const double3 &rtp)
{ // spherical coordinates (r, theta, phi) -> cartesian coordiantes (x, y, z)
	const double rsint = rtp.x * sin(rtp.y);
	return double3(rsint * cos(rtp.z), rsint * sin(rtp.z), rtp.x * cos(rtp.y));
}
inline double3 rtp(const double3 &xyz)
{ // cartesian coordiantes (x, y, z) -> spherical coordinates (r, theta, phi)
	const double r = length(xyz);
	return double3(r, acos(xyz.z / r), atan2(xyz.y, xyz.x));
}
inline double plane_distance(const double3 &p, const double3 &plane_point, const double3 &plane_normal)
{ // returns distance of point p from plane
	return dot(plane_normal, p - plane_point);
}
...
{ // 3D matrix type
	...
	double3x3(const double xx, const double yy, const double zz)
	{ // create diagonal matrix
		this->xx = xx;
		this->yy = yy;
		this->zz = zz;
	...
	double3x3(const double xx, const double xy, const double xz, const double yx, const double yy, const double yz, const double zx, const double zy, const double zz)
	{ // create matrix
		this->xx = xx;
		this->xy = xy;
		this->xz = xz;
	...
	double3x3(const double3 &v1, const double3 &v2)
	{ // create matrix from dyadic product of two vectors
		this->xx = v1.x * v2.x;
		this->xy = v1.x * v2.y;
		this->xz = v1.x * v2.z;
	...
	double3x3(const double3 &v1, const double3 &v2, const double3 &v3)
	{ // create matrix from three column vectors
		this->xx = v1.x;
		this->xy = v2.x;
		this->xz = v3.x;
	...
	double3x3(const double3 &v, const double r)
	{ // create rotation matrix for a rotation around the (normalized) axis v with angle r in radians
		const double sr = sin(r), cr = cos(r);
		this->xx = sq(v.x) + (1.0 - sq(v.x)) * cr;
		this->xy = v.x * v.y * (1.0 - cr) - v.z * sr;
		this->xz = v.x * v.z * (1.0 - cr) + v.y * sr;
		this->yx = v.x * v.y * (1.0 - cr) + v.z * sr;
		this->yy = sq(v.y) + (1.0 - sq(v.y)) * cr;
		this->yz = v.y * v.z * (1.0 - cr) - v.x * sr;
		this->zx = v.x * v.z * (1.0 - cr) - v.y * sr;
		this->zy = v.y * v.z * (1.0 - cr) + v.x * sr;
		this->zz = sq(v.z) + (1.0 - sq(v.z)) * cr;
	}
	double3x3() = default;
	...
	inline double3x3 &operator+=(const double3x3 &m)
	{ // elementwise addition
		this->xx += m.xx;
		this->xy += m.xy;
		this->xz += m.xz;
	...
	inline double3x3 &operator+=(const double x)
	{ // elementwise addition
		this->xx += x;
		this->xy += x;
		this->xz += x;
	...
	inline double3x3 operator^(const uint n) const
	{																				 // matrix power
...
inline double3x3 operator+(const double3x3 &m, const double x)
{ // elementwise addition
	return double3x3(
			m.xx + x, m.xy + x, m.xz + x,
			m.yx + x, m.yy + x, m.yz + x,
...
inline double3x3 operator+(const double x, const double3x3 &m)
{ // elementwise addition
	return m + x;
}
inline double3x3 operator-(const double3x3 &m, const double x)
...
inline double3 operator*(const double3 &v, const double3x3 &m)
{ // multiply vector with matrix
	return double3(v.x * m.xx + v.y * m.yx + v.z * m.zx, v.x * m.xy + v.y * m.yy + v.z * m.zy, v.x * m.xz + v.y * m.yz + v.z * m.zz);
}
inline double3 operator*(const double3x3 &m, const double3 &v)
{ // multiply matrix with vector
	return double3(m.xx * v.x + m.xy * v.y + m.xz * v.z, m.yx * v.x + m.yy * v.y + m.yz * v.z, m.zx * v.x + m.zy * v.y + m.zz * v.z);
}
inline double3::double3(const double3x3 &m)
{ // extract diagonal of matrix
	this->x = m.xx;
	this->y = m.yy;
	this->z = m.zz;
}
inline double3 &double3::operator=(const double3x3 &m)
{ // extract diagonal of matrix
	this->x = m.xx;
	this->y = m.yy;
	this->z = m.zz;
...
struct doubleN
{
	...
	doubleN(const uint N, const double x = 0.0)
	{ // create vector filled with zeros
		this->N = N;
		this->V = new double[N];
		for (uint i = 0u; i < N; i++)
	...
	inline doubleN &operator+=(const double x)
	{ // elementwise addition
		for (uint i = 0u; i < N; i++)
			this->V[i] += x;
		return *this;
	...
	inline doubleN operator+(const doubleN &v) const
	{ // elementwise addition
		doubleN r = doubleN(N);
		for (uint i = 0u; i < N; i++)
			r.V[i] = this->V[i] + v.V[i];
	...
	inline double operator*(const doubleN &v) const
	{ // dot product
		double r = 0.0;
		for (uint i = 0u; i < N; i++)
			r += (double)this->V[i] * (double)v.V[i];
...
inline doubleN operator+(const doubleN &v, const double x)
{ // elementwise addition
...
inline doubleN operator+(const double x, const doubleN &v)
{ // elementwise addition
	return v + x;
}
inline doubleN operator-(const doubleN &v, const double x)
...
struct doubleNxN
{
	...
	doubleNxN(const uint N, const double x = 0.0)
	{ // create matrix filled with zeros
		this->N = N;
	...
	inline doubleNxN &operator=(const doubleN &v)
	{ // create diagonal matrix from vector
		delete[] this->M;
		this->N = v.N;
		this->M = new double[v.N * v.N];
	...
	inline doubleNxN &operator+=(const double x)
	{ // elementwise addition
		for (uint i = 0u; i < N * N; i++)
			this->M[i] += x;
		return *this;
	...
	inline doubleNxN operator-() const
	{ // elementwise negation
	...
	inline doubleNxN operator+(const doubleNxN &m) const
	{ // elementwise addition
		doubleNxN r = doubleNxN(N);
		for (uint i = 0u; i < N * N; i++)
			r.M[i] = this->M[i] + m.M[i];
	...
	inline doubleNxN operator^(const uint n) const
	{																									 // matrix power
	...
	{ // returns inverse matrix
		...
		for (uint i = 0u; i < N; i++)
		{
			const double t = A[2u * N * i + i];
			for (uint j = 0u; j < N; j++)
				r.M[N * i + j] = A[2u * N * i + N + j] / t;
		}
		delete[] A;
		return r;
	}
	inline doubleNxN transpose() const
	{ // returns transpose matrix
		doubleNxN r = doubleNxN(N);
		for (uint i = 0u; i < N; i++)
			for (uint j = 0u; j < N; j++)
...
inline doubleNxN operator+(const doubleNxN &m, const double x)
{ // elementwise addition
...
inline doubleNxN operator+(const double x, const doubleNxN &m)
{ // elementwise addition
	return m + x;
}
inline doubleNxN operator-(const doubleNxN &m, const double x)
...
inline doubleN operator*(const doubleN &v, const doubleNxN &m)
{ // multiply matrix with vector
	doubleN r = doubleN(v.N);
	for (uint i = 0u; i < v.N; i++)
	{
...
inline doubleN operator*(const doubleNxN &m, const doubleN &v)
{ // multiply vector with matrix
...
inline doubleN::doubleN(const uint N, const doubleNxN &m)
{ // extract diagonal of matrix
	this->N = m.N;
...
inline doubleN &doubleN::operator=(const doubleNxN &m)
{ // extract diagonal of matrix
	delete[] this->V;
	this->N = m.N;
	this->V = new double[m.N];
...
inline float plic_cube_reduced(const float V, const float n1, const float n2, const float n3)
{ // optimized solution from SZ and Kawano
	const float n12 = n1 + n2, n3V = n3 * V;
	if (n12 <= 2.0f * n3V)
		return n3V + 0.5f * n12;																	 // case (5)
	const float sqn1 = sq(n1), n26 = 6.0f * n2, v1 = sqn1 / n26; // after case (5) check n2>0 is true
	if (v1 <= n3V && n3V < v1 + 0.5f * (n2 - n1))
		return 0.5f * (n1 + sqrt(sqn1 + 8.0f * n2 * (n3V - v1))); // case (2)
	const float V6 = n1 * n26 * n3V;
	if (n3V < v1)
		return cbrt(V6);																																																												// case (1)
...
inline float plic_cube(const float V0, const float3 n)
{																																																						// unit cube - plane intersection: volume V0 in [0,1], normal vector n -> plane offset d0
...
inline float plic_cube_inverse(const float d0, const float3 n)
{																																// unit cube - plane intersection: plane offset d0, normal vector n -> volume V0 in [0,1]
...
inline float plic_sphere(const float V0)
{																																																				 // intersection of a sphere with volume 1 and a plane: truncaterd volume V0 -> plane offset d0
...
inline float plic_sphere_inverse(const float d0)
{																															// intersection of a sphere with volume 1 and a plane: plane offset d0 -> truncaterd volume V0
...
inline void split_float(float x, uint &integral, uint &decimal, int &exponent)
{
	...
	if (remainder - (float)decimal >= 0.5f)
	{ // correct rounding of last decimal digit
		decimal++;
		if (decimal >= 100000000u)
		{ // decimal overflow
			decimal = 0u;
			integral++;
			if (integral >= 10u)
			{ // decimal overflow causes integral overflow
				integral = 1u;
				exponent++;
			}
		}
...
inline void split_double(double x, uint &integral, ulong &decimal, int &exponent)
{
	...
	if (remainder - (double)decimal >= 0.5)
	{ // correct rounding of last decimal digit
		decimal++;
		if (decimal >= 10000000000000000ull)
		{ // decimal overflow
			decimal = 0ull;
			integral++;
			if (integral >= 10u)
...
{ // convert float to string with full precision (<string> to_string() prints only 6 decimals)
	string s = "";
	if (x < 0.0f)
	{
		s += "-";
...
{ // convert double to string with full precision (<string> to_string() prints only 6 decimals)
	string s = "";
	if (x < 0.0)
	{
		s += "-";
...
inline string to_string(float x, const uint decimals)
{ // convert float to string with specified number of decimals
	...
	if (std::isnan(x))
		return s + "NaN";
	if (std::isinf(x) || x > (float)max_ulong)
		return s + "Inf";
...
inline string to_string(double x, const uint decimals)
{ // convert float to string with specified number of decimals
	string s = "";
	if (x < 0.0)
	{
	...
	if (std::isnan(x))
		return s + "NaN";
	if (std::isinf(x) || x > (double)max_ulong)
		return s + "Inf";
...
{ // this vector notation is needed when working with Configuration_File
	string s = "[";
...
{
	for (uint i = 0u; i < (uint)matches.size(); i++)
		if (contains(s, matches[i]))
			return true;
	return false;
}
...
inline bool begins_with(const string &s, const string &match)
{
	if (match.size() > s.size())
		...
		return equal(match.begin(), match.end(), s.begin());
}
...
inline string alignl(const uint n, const string &x = "")
...
inline string alignr(const uint n, const string &x = "")
{ // converts x to string with spaces in front such that length is n if x is not longer than n
	string s = "";
	for (uint i = 0u; i < n; i++)
		s += " ";
	s += x;
...
template <typename T>
inline string alignl(const uint n, const T x)
{ // converts x to string with spaces behind such that length is n if x does not have more digits than n
	return alignl(n, to_string(x));
}
template <typename T>
inline string alignr(const uint n, const T x)
{ // converts x to string with spaces in front such that length is n if x does not have more digits than n
	return alignr(n, to_string(x));
}

inline string print_time(const double t)
{ // input: time in seconds, output: formatted string ___y___d__h__m__s
	uint s = (uint)(t + 0.5);
	const uint y = s / 31556925;
	s %= 31556925;
	const uint d = s / 86400;
	s %= 86400;
	const uint h = s / 3600;
	s %= 3600;
	const uint m = s / 60;
...
inline Image *rescale(const Image *image, const uint newwidth, const uint newheight, Image *rescaled = nullptr)
{
	...
	{ // scale down (average downscaling)
	...
	{ // x: average downscaling, y: integer upscaling
		for (uint ny = 0u; ny < newheight; ny++)
		{
			for (uint nx = 0u; nx < newwidth; nx++)
			{
	...
	{ // x: integer upscaling, y: average downscaling
		for (uint ny = 0u; ny < newheight; ny++)
		{
			for (uint nx = 0u; nx < newwidth; nx++)
			{
...
inline int invert(const int color)
{ // invert color
	return ::color(255 - red(color), 255 - green(color), 255 - blue(color));
}
inline int invert_brightness(const int color)
{ // invert brightness, but retain color
...
inline float3 rgb_to_hsv(const int red, const int green, const int blue)
{
	...
	const float h = 60.0f * (d == 0 ? 0.0f : cmax == red ? fmod((float)(green - blue) / fd + 6.0f, 6.0f)
																			 : cmax == green ? (float)(blue - red) / fd + 2.0f
																											 : (float)(red - green) / fd + 4.0f);
...
{ // returns path where executable is located, ends with a "/"
	string path = "";
#if defined(_WIN32)
	wchar_t wc[260] = {0};
	GetModuleFileNameW(NULL, wc, 260);
	std::wstring ws(wc);
	transform(ws.begin(), ws.end(), back_inserter(path), [](wchar_t c)
						{ return (char)c; });
	path = replace(path, "\\", "/");
#else	 // Linux
	char c[260];
...
inline string get_textcolor_code(const int textcolor)
{ // Linux only
	switch (textcolor)
	{
	case 0:
...
inline string get_backgroundcolor_code(const int backgroundcolor)
{ // Linux only
...
inline string get_print_color(const int textcolor)
{ // Linux only
	return "\033[" + get_textcolor_code(textcolor) + "m";
}
inline string get_print_color(const int textcolor, const int backgroundcolor)
{ // Linux only
...
inline void print_no_reset(const string &s, const int textcolor)
{ // print with color, but don't reset color afterwards (faster)
	print_color(textcolor);
	std::cout << s;
}
inline void print_no_reset(const string &s, const int textcolor, const int backgroundcolor)
{ // print with color, but don't reset color afterwards (faster)
	print_color(textcolor, backgroundcolor);
	std::cout << s;
}
...
inline void print_image_bw(const Image *image, const uint textwidth = 0u, const uint textheight = 0u)
...
{
	for (int i = k * 16777216 / N; i < (k + 1) * 16777216 / N; i++)
		print_image_dither_lookup[i] = get_console_color_dither(i);
}
inline void print_image_dither_initialize_lookup()
{ // initialize lookup table parallelized (much faster)
	if (!print_image_dither_lookup_initialized)
	{
		...
		for (int k = 0; k < N; k++)
			threads[k] = thread(print_image_dither_initialize_lookup_thread, k, N);
		for (int k = 0; k < N; k++)
			threads[k].join();
		delete[] threads;
...
inline void print_video_dither(const Image *image, const uint textwidth = 0u, const uint textheight = 0u)
{ // optimized for video (only draws pixels that differ from last frame)
#ifdef UTILITIES_CONSOLE_DITHER_LOOKUP
	print_image_dither_initialize_lookup();
#endif // UTILITIES_CONSOLE_DITHER_LOOKUP
	uint newwidth = (uint)(CONSOLE_WIDTH), newheight = (uint)(CONSOLE_WIDTH)*9u / 16u;
	...
	for (uint i = 0u; i < newwidth * newheight; i++)
	{
		if (print_video_lastframe_is_new || print_image_rescaled->color(i) != print_video_lastframe->color(i))
			print_video_lastframe->set_color(changed_pixels++, (int)i);
	}
	if (changed_pixels > 0u)
	{ // at least one pixel has changed
#if defined(_WIN32)
		const string s[3] = {string("") + (char)176, string("") + (char)177, string("") + (char)178};
		...
		for (uint i = 1u; i < changed_pixels; i++)
		{
			...
			const int shade = dither >> 8;
			const int tc = (dither >> 4) & 0xF, bc = dither & 0xF;
			if (tc != ltc || bc != lbc || x != lx + 1u || y != ly)
			{																		 // color has changed or cursor has moved
...
inline bool equals_regex(const string &s, const string &match)
{ // returns true if string exactly matches regex
	return regex_match(s.begin(), s.end(), std::regex(match));
}
inline uint matches_regex(const string &s, const string &match)
...
inline void print_message(const string &message, const string &keyword = "")
...
inline void print_warning(const string &s)
{ // print formatted warning message
	print_message(s, "Warning: ");
}
inline void print_info(const string &s)
...
{
	if (!equals_regex(s, regex))
		print_error("\"" + s + "\" cannot be parsed to " + type + ".");
}
...
inline void create_folder(const string &path)
{																									 // create folder if it not already exists
	...
	if (slash_position == (int)string::npos)
		return;																				 // no slash found
	const string f = path.substr(0, slash_position); // cut off file name if there is any
#ifndef UTILITIES_NO_CPP17
	if (!std::filesystem::is_directory(f) || !std::filesystem::exists(f))
		std::filesystem::create_directories(f); // create folder if it not already exists
#endif																			// UTILITIES_NO_CPP17
}
...
template <typename T>
inline void write_file(const string &filename, const string &header, const uint n, const T *y)
{
	string s = header;
	if (length(s) > 0u && !ends_with(s, "\n"))
		s += "\n";
	for (uint i = 0u; i < n; i++)
		s += to_string(i) + "\t" + to_string(y[i]) + "\n";
	write_file(filename, s);
}
template <typename T, typename U>
inline void write_file(const string &filename, const string &header, const uint n, const T *x, const U *y)
{
	string s = header;
	if (length(s) > 0u && !ends_with(s, "\n"))
		s += "\n";
...
inline Image *read_bmp(const string &filename, Image *image = nullptr)
{
	...
	if (filesize != 54u + imagesize)
		println("\rError: File \"" + filename + "\" is corrupt or unsupported!");
	if (image == nullptr || image->width() != width || image->height() != height)
	{
		delete image;
		image = new Image(width, height);
...
inline void write_bmp(const string &filename, const Image *image)
{
	...
	for (uint y = 0u; y < image->height(); y++)
	{
		...
		for (uint p = 0u; p < pad; p++)
			data[54 + (3u * image->width() + p) + y * (3u * image->width() + pad)] = 0;
	}
...
inline Image *read_qoi(const string &filename, Image *image = nullptr)
{ // 4-channel .qoi decoder, source: https://qoiformat.org/qoi-specification.pdf
	...
	file.read((char *)data, filesize);
	file.close();
	if (filesize < 22u || ((int *)data)[0] != 1718185841)
		println("\rError: File \"" + filename + "\" is corrupt!");
	uint width = 0u, height = 0u;
	for (uint i = 0u; i < 4u; i++)
	{
	...
	for (uint i = 0u; i < image->length() && counter < filesize - 8u; i++)
	{
		...
		else
		{
			...
			else if (tag == 0b00000000)
			{												 // lookup table encoding (1 byte)
				pixel = lookup[chunk]; // chunk&0b00111111
			}
			else if (tag == 0b01000000)
			{ // difference encoding (1 byte)
...
inline void write_qoi(const string &filename, const Image *image)
{ // 3-channel .qoi encoder, source: https://qoiformat.org/qoi-specification.pdf
	create_folder(filename);
	...
	for (uint i = 0u; i < image->length(); i++)
	{
		...
		else
		{
			...
			else
			{
				...
				else if (dg >= -32 && dg < 32 && drdg >= -8 && drdg < 8 && dbdg >= -8 && dbdg < 8)
				{ // luma encoding (2 bytes)
...
inline Mesh *read_stl(const string &path, const float3 &box_size, const float3 &center, const float3x3 &rotation, const float size)
...
private:
	...
	{
		if ((values.front() == '{' && values.back() == '}') || (values.front() == '[' && values.back() == ']') || (values.front() == '(' && values.back() == ')') || (values.front() == '<' && values.back() == '>'))
			values = values.substr(1, values.length() - 2);
		vector<string> elements = split_regex(values, "\\s*[,;]\\s*");
		for (uint j = 0u; j < (uint)elements.size(); j++)
			elements[j] = trim(elements[j]);
		return elements;
	}
	template <class T>
	inline T from_string(const string &s, const string &name) const
	{
		if constexpr (std::is_same<T, bool>::value)
			return contains(to_lower(s), "true") ? (T)1 : contains(to_lower(s), "false") ? (T)0
																																									 : (T)to_int(s);
		else if constexpr (std::is_same<T, char>::value)
			return (T)to_int(s);
		else if constexpr (std::is_same<T, uchar>::value)
			return (T)to_uint(s);
			...
			return (T)to_uint(s);
		else if constexpr (std::is_same<T, slong>::value)
			return (T)to_slong(s);
			...
			return (T)to_double(s);
		else if constexpr (std::is_same<T, string>::value)
			return (s.front() == '"' && s.back() == '"') ? s.substr(1, s.length() - 2) : s;
		else
			print_error("Generic type in call value<T>(\"" + name + "\") is not supported.");
		return T(); // otherwise compiler complains about missing return statement
	}
	template <typename T>
	inline bool extract_value(const string &name, T &v) const
	{
	...
	template <typename T>
	inline bool extract_value(const string &name, vector<T> &v) const
	{
		for (uint i = 0u; i < (uint)entries.size(); i++)
		{
			if (entries[i].name == name)
			{
				...
				for (uint j = 0u; j < (uint)elements.size(); j++)
					v.push_back(from_string<T>(elements[j], name));
				return true;
			}
		}
		return false;
...
public:
	inline Configuration_File(const string &filename)
	{
		...
		for (uint i = 0u; i < (uint)lines.size(); i++)
		{
			...
			if (length(lines[i]) == 0u || begins_with(lines[i], "//") || lines[i].front() == '#' || (lines[i].front() == '[' && lines[i].back() == ']'))
				lines.erase(lines.begin() + i); // remove comment lines
		}
		for (uint i = 0u; i < (uint)lines.size(); i++)
		{
			const vector<string> data = split_regex(lines[i], "\\s*=\\s*"); // split lines in "name = value"
			if ((int)data.size() != 2)
				print_error("Entry \"" + lines[i] + "\" in configuration file \"" + filename + "\" is invalid.");
			entries.push_back(Entry(trim(data[0]), trim(data[1])));
		}
	}
	template <class T>
	inline T value(const string &name) const
	{ // valid for bool, char, uchar, short, ushort, int, uint, slong, ulong, float, double, string and vectors of these types
		T t;
		if (extract_value(name, t))
			return t;
		else
			print_error("There is no entry called \"" + name + "\" in configuration file \"" + filename + "\".");
	}
	template <class T>
	inline T value(const string &name, const T &default_value) const
	{ // valid for bool, char, uchar, short, ushort, int, uint, slong, ulong, float, double, string and vectors of these types
		T t;
		if (extract_value(name, t))
			return t;
