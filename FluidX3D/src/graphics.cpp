...
bool convert(int &rx, int &ry, float &rz, const float3 &p, const int stereo)
{ // 3D -> 2D
	float3 t, r;
	...
	if (rs <= 0.0f)
		return false; // point is behins camera
...
bool is_off_screen(const int x, const int y)
...
{ // check if point is off screen
	...
	default:
		return x < 0 || x >= (int)camera.width || y < 0 || y >= (int)camera.height; // entire screen
	case -1:
		return x < 0 || x >= (int)camera.width / 2 || y < 0 || y >= (int)camera.height; // left half
	case +1:
		return x < (int)camera.width / 2 || x >= (int)camera.width || y < 0 || y >= (int)camera.height; // right half
	}
}
bool intersect_lines(const int x0, const int y0, const int x1, const int y1, const int xA, const int yA, const int xB, const int yB)
...
bool intersect_line_rectangle(const int x0, const int y0, const int x1, const int y1, const int xA, const int yA, const int xB, const int yB)
...
bool intersects_screen(const int x0, const int y0, const int x1, const int y1, const int stereo)
{
	...
	{
	case 0:
		return intersect_line_rectangle(x0, y0, x1, y1, 0, 0, camera.width, camera.height);
	case -1:
		return intersect_line_rectangle(x0, y0, x1, y1, 0, 0, camera.width / 2, camera.height);
	case +1:
		return intersect_line_rectangle(x0, y0, x1, y1, camera.width / 2, 0, camera.width, camera.height);
	}
	return false;
}
...
int color_mix_3(const int c0, const int c1, const int c2, const float w0, const float w1, const float w2)
{ // w0+w1+w2 = 1
...
void convert_circle(const float3 &p, const float r, const int color, const int stereo)
{
	...
	if (convert(rx, ry, rz, p, stereo))
	{
		...
		{
		default:
			if (rx < -radius || rx >= (int)camera.width + radius || ry < -radius || ry >= (int)camera.height + radius)
				return;
			break; // cancel drawing if circle is off screen
		case -1:
			if (rx < -radius || rx >= (int)camera.width / 2 + radius || ry < -radius || ry >= (int)camera.height + radius)
				return;
			break;
		case +1:
			if (rx < (int)camera.width / 2 - radius || rx >= (int)camera.width + radius || ry < -radius || ry >= (int)camera.height + radius)
				return;
			break;
		}
...
void convert_line(const float3 &p0, const float3 &p1, const int color, const int stereo)
{
	int r0x, r0y, r1x, r1y;
	float r0z, r1z;
	if (convert(r0x, r0y, r0z, p0, stereo) && convert(r1x, r1y, r1z, p1, stereo) && !(is_off_screen(r0x, r0y, stereo) && is_off_screen(r1x, r1y, stereo) && !intersects_screen(r0x, r0y, r1x, r1y, stereo)))
	{
...
void convert_triangle(const float3 &p0, const float3 &p1, const float3 &p2, const int color, const int stereo)
{
	...
	float r0z, r1z, r2z;
	if (convert(r0x, r0y, r0z, p0, stereo) && convert(r1x, r1y, r1z, p1, stereo) && convert(r2x, r2y, r2z, p2, stereo) && !(is_off_screen(r0x, r0y, stereo) && is_off_screen(r1x, r1y, stereo) && is_off_screen(r2x, r2y, stereo) && !intersects_screen(r0x, r0y, r1x, r1y, stereo) && !intersects_screen(r1x, r1y, r2x, r2y, stereo) && !intersects_screen(r2x, r2y, r0x, r0y, stereo)))
	{
		if (r0y == r1y && r0y == r2y)
			return; // return for degenerate triangles
		if (r0y > r1y)
		{
			std::swap(r0x, r1x);
			std::swap(r0y, r1y);
		...
		{ // Bresenham algorithm (lower triangle half)
		...
		{ // Bresenham algorithm (upper triangle half)
			const int xA = r0x + (r2x - r0x) * (y - r0y) / (r2y - r0y);
...
void convert_text(const float3 &p, const string &s, const float r, const int color, const int stereo)
{
	...
	if (convert(rx, ry, rz, p, stereo))
	{
		...
		for (int i = 0u; i < (int)length(s); i++)
		{
			...
			{
				if ((pixels >> (63 - k)) & 1)
					draw(rx + i * 6 + k % 6 + (character == 113), ry + k / 6, rz, color, stereo);
			}
		}
	}
}
...
void draw_pixel(const int x, const int y, const int color)
{
	if (!is_off_screen(x, y))
		camera.bitmap[x + y * (int)camera.width] = color; // only draw if point is on screen
}
...
void draw_text(const int x, const int y, const string &s, const int color)
{
	for (int i = 0; i < (int)length(s); i++)
	{
		...
		{
			if ((pixels >> (63 - k)) & 1)
				draw_pixel(x + i * 6 + k % 6 + (character == 113), y + k / 6, color);
		}
	}
}
...
void draw_triangle(const float3 &p0, const float3 &p1, const float3 &p2, const int color, const bool translucent)
...
void draw_triangle(const float3 &p0, const float3 &p1, const float3 &p2, const int c0, const int c1, const int c2, const bool translucent)
{ // points clockwise from above
...
{ // call WinMain from dummy main function in order to have an additional console window
...
int key_linux(const int keycode)
{
	...
	case 66:
		return -20; // caps lock
	case 115:
		return -91; // windows key
	case 117:
		return -93; // kontext menu key
	case 87:
		return '1';
	case 85:
		return '6'; // numpad
	...
	case 98:
		return -38;
	case 104:
		return -40; // up/down arrow
	case 100:
		return -37;
	case 102:
		return -39; // left/right arrow
	case 99:
		return -33;
	case 105:
		return -34; // page up/down
	case 97:
		return -36;
	case 103:
		return -35; // pos1/end
	...
	while (running)
	{
		if (!updating_frame)
		{
			...
			else if (x11_event.type == ButtonPress)
			{
				...
				if (x11_button == Button4)
				{ // scroll up
					camera.input_scroll_up();
				}
				else if (x11_button == Button5)
				{ // scroll down
					camera.input_scroll_down();
				}
				else
...
int main(int argc, char *argv[])
{
	...
	if (!x11_display)
		print_error("No X11 display available.");
	...
	struct Hints
	{
		long flags = 2l, functions = 0l, decorations = 0b0111010l, input_mode = 0l, status = 0l;
	} x11_hints; // decorations=maximize|minimize|menu|title|resize|border|all
...
void update_frame(const double frametime)
{
	if (last_textwidth == 0u && last_textheight == 0u)
