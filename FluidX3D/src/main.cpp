...
void main_label(const double frametime) {
	if(info.allow_rendering) {
		...
		const int c = color(255-red(GRAPHICS_BACKGROUND_COLOR), 255-green(GRAPHICS_BACKGROUND_COLOR), 255-blue(GRAPHICS_BACKGROUND_COLOR));
		{
			...
			draw_label(ox, oy+i, "MLUPs "          +alignr(31u, /**********************/ alignr(5u, to_uint(pn*1E-6/info.dt_smooth))+" ("+alignr(5u, to_uint(pn*mt*1E-9/info.dt_smooth))+"    GB/s)"), c); i+=FONT_HEIGHT;
			draw_label(ox, oy+i, "Steps "          +alignr(31u, /*******************************************/ alignr(10u, info.lbm->get_t())+" ("+alignr(5, to_uint(1.0/info.dt_smooth))+" Steps/s)"), c); i+=FONT_HEIGHT;
