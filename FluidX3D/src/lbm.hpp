...
public:
	...
	void set_f(const float fx, const float fy, const float fz)
	{
		set_fx(fx);
		set_fy(fy);
		set_fz(fz);
...
#ifdef SURFACE
		const string path_skybox = get_exe_path() + "../skybox/skybox.png";
...
public:
	...
	{ // does not hold any data itsef, just links to LBM_Domain data
	private:
		LBM *lbm = nullptr;
		...
		inline T &reference(const ulong i, const uint dimension)
		{ // stitch together domain buffers and make them appear as one single large buffer
		...
		inline static string vtk_type()
		{
			/**/ if constexpr (std::is_same<T, char>::value)
				...
				print_error("Error in vtk_type(): Type not supported.");
			return "";
		}
		inline void write_vtk(const string &path)
		{ // write binary .vtk file
			...
			const string header =
					...
					to_string(Nx) + " " + to_string(Ny) + " " + to_string(Nz) + "\n"
																																			"ORIGIN " +
					to_string(origin.x) + " " + to_string(origin.y) + " " + to_string(origin.z) + "\n"
																																												"SPACING " +
					to_string(spacing) + " " + to_string(spacing) + " " + to_string(spacing) + "\n"
																																										 "POINT_DATA " +
					to_string((ulong)Nx * (ulong)Ny * (ulong)Nz) + "\nSCALARS data " + vtk_type() + " " + to_string(dimensions()) + "\nLOOKUP_TABLE default\n";
			T *data = new T[range()];
	...
	public:
		...
		inline Memory_Container(LBM *lbm, Memory<T> **buffers, const string &name)
		{
			...
			if (this->N * (ulong)this->d == 0ull)
				print_error("Memory size must be larger than 0.");
			initialize_auxiliary_pointers();
		}
		inline Memory_Container() {} // default constructor
		inline Memory_Container &operator=(Memory_Container &&memory) noexcept
		{ // move assignment
			this->lbm = memory.lbm;
			this->Nx = memory.Nx;
			this->Ny = memory.Ny;
		...
		inline void reset(const T value = (T)0)
		{
			...
			{
				for (ulong i = 0ull; i < range() / (ulong)(Dx * Dy * Dz); i++)
					buffers[domain][i] = value;
			}
			write_to_device();
		}
		...
		inline void read_from_device()
		{
			...
			for (uint domain = 0u; domain < Dx * Dy * Dz; domain++)
				lbm->lbm[domain]->enqueue_update_fields(); // make sure data in device memory is up-to-date
#endif																						 // UPDATE_FIELDS
			for (uint domain = 0u; domain < Dx * Dy * Dz; domain++)
				buffers[domain]->enqueue_read_from_device();
			for (uint domain = 0u; domain < Dx * Dy * Dz; domain++)
				buffers[domain]->finish_queue();
		}
		inline void write_to_device()
		{
			for (uint domain = 0u; domain < Dx * Dy * Dz; domain++)
				buffers[domain]->enqueue_write_to_device();
			for (uint domain = 0u; domain < Dx * Dy * Dz; domain++)
				buffers[domain]->finish_queue();
		}
		inline void write_host_to_vtk(const string &path = "")
		{ // write binary .vtk file
			write_vtk(default_filename(path, name, ".vtk", lbm->get_t()));
		}
		inline void write_device_to_vtk(const string &path = "")
		{ // write binary .vtk file
			read_from_device();
	...
	void set_fy(const float fy)
	{
		for (uint d = 0u; d < get_D(); d++)
			lbm[d]->set_fy(fy);
	} // set global froce per volume
	void set_fz(const float fz)
	{
		for (uint d = 0u; d < get_D(); d++)
			lbm[d]->set_fz(fz);
	} // set global froce per volume
	...
	{																							 // disassemble 1D linear index to 3D coordinates (n -> x,y,z)
		const ulong t = n % ((ulong)Nx * (ulong)Ny); // n = x+(y+z*Ny)*Nx
		x = (uint)(t % (ulong)Nx);
		y = (uint)(t / (ulong)Nx);
		z = (uint)(n / ((ulong)Nx * (ulong)Ny));
	}
	void coordinates(const float3 &p, uint &x, uint &y, uint &z) const
	{ // turn 3D position into closest 3D grid coordinates
		const float3 mp = mirror_position(p);
		x = (uint)(mp.x + 1.5f * (float)Nx) % Nx;
		y = (uint)(mp.y + 1.5f * (float)Ny) % Ny;
		z = (uint)(mp.z + 1.5f * (float)Nz) % Nz;
	}
	ulong index(const uint x, const uint y, const uint z) const
	{ // turn 3D coordinates into 1D linear index
		return (ulong)x + ((ulong)y + (ulong)z * (ulong)Ny) * (ulong)Nx;
	}
	ulong index(const float3 &p) const
	{ // turn 3D position into closest 1D linear index
		uint x = 0u, y = 0u, z = 0u;
		coordinates(p, x, y, z);
		return index(x, y, z);
	}
	float3 position(const uint x, const uint y, const uint z) const
	{ // returns position in box [-Nx/2, Nx/2] x [-Ny/2, Ny/2] x [-Nz/2, Nz/2]
		return float3((float)x - 0.5f * (float)Nx + 0.5f, (float)y - 0.5f * (float)Ny + 0.5f, (float)z - 0.5f * (float)Nz + 0.5f);
	}
	float3 position(const ulong n) const
	{ // returns position in box [-Nx/2, Nx/2] x [-Ny/2, Ny/2] x [-Nz/2, Nz/2]
		uint x, y, z;
		coordinates(n, x, y, z);
		return position(x, y, z);
	}
	float3 mirror_position(const float3 &p) const
	{ // mirror position into periodic boundaries
		float3 r;
	...
	float3 relative_position(const uint x, const uint y, const uint z) const
	{ // returns relative position in box [-0.5, 0.5] x [-0.5, 0.5] x [-0.5, 0.5]
		return float3(((float)x + 0.5f) / (float)Nx - 0.5f, ((float)y + 0.5f) / (float)Ny - 0.5f, ((float)z + 0.5f) / (float)Nz - 0.5f);
	}
	float3 relative_position(const ulong n) const
	{ // returns relative position in box [-0.5, 0.5] x [-0.5, 0.5] x [-0.5, 0.5]
		uint x, y, z;
		coordinates(n, x, y, z);
	...
	void voxelize_array_representation(uchar *data, const float3 &center, const float3x3 &rotation, const float size, const uchar flag = TYPE_S);
	...
	public:
		...
		~Graphics()
		{ // destructor must wait for all encoder threads to finish
